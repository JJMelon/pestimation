...
class ThreePhaseParser:
    ...
    def create_buses(self, network_model: DxNetworkModel):
        # Read all ditto models
        for model in self.ditto_store.models:
            # Check if model is Node or Voltage Source
            if isinstance(model, ditto.models.node.Node) or isinstance(model, ditto.models.power_source.PowerSource):
                # Check if the model is slack
                isSlack = False
                if isinstance(model, ditto.models.power_source.PowerSource) or (hasattr(model, "bustype") and model._bustype == "SWING"):
                    isSlack = True
                if "A" in model.phases or "B" in model.phases or "C" in model.phases:
                    if model.parent != None:
                        ThreePhaseParser.bus2isTriplex[model.parent] = 0
                    ThreePhaseParser.bus2isTriplex[model.name] = 0
                elif "1" in model.phases or "2" in model.phases:
                    if model.parent != None:
                        ThreePhaseParser.bus2isTriplex[model.parent] = 1
                ...
                for phase in model.phases:
                    ...
                    if phase in self._phase_to_angle:
                        ...
                        if model.parent != None:
                            ...
                            bus = self.create_bus(network_model, v_mag, v_ang, model.name, model.parent, phase, False)
    ...
    def create_bus(self, network_model, v_mag, v_ang, node_name, node_parent, node_phase, is_virtual):
    ...
    def create_load(self, model, network_model: DxNetworkModel):
        ...
        for phase_load in model.phase_loads:
            ...
            pq_load = Load(model.name, from_bus, to_bus, phase_load.p, phase_load.q, phase_load.z, ip, iq, load_num, phase_load.phase, triplex_phase)
    ...
    def create_transmission_line(self, model: ditto.models.line.Line, network_model: DxNetworkModel):
        ...
        if model.is_fuse:
            for wire in model.wires:
                ...
                #fuse_bus = self.create_bus(network_model, 0.1, 0.1, f"{from_bus.NodeName}-Fuse", \
                #                           None, wire.phase, True)
        ...
        # Check for switches, some are encoded as 1-length lines with no features
            ...
            #print(impedances)
            # self.network_impedance = impedances
            if self.settings.enable_line_capacitance:
                shunt_admittances = model.capacitance_matrix
            else:
                shunt_admittances = None

            if model.impedance_matrix_exists:
                phases = []
                if impedances[0, 0] != 0 + 0j:
                    if self.bus2isTriplex[model.from_element] == 0:
                        phases.append("A")
                    else:
                        phases.append("1")
                if impedances[1, 1] != 0 + 0j:
                    if self.bus2isTriplex[model.from_element] == 0:
                        phases.append('B')
                    else:
                        phases.append("2")
                if impedances[2, 2] != 0 + 0j:
                    phases.append('C')
            else:
