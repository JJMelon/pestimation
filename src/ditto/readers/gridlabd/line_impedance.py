...
def compute_overhead_capacitance(wire_list, distances, freq = 60):
    ...
    # Y = Y / 2 # We are doing this in the main code
...
def compute_underground_capacitance(wire_list):
    ...
    #Shunt capacitance is applied on either side of the line.
    # capacitance_matrix = capacitance_matrix / 2 # We are doing this in the main code
...
def try_load_direct_line_capacitance(line_config):
    ...
    for property_number in line_direct_lookup.keys():
        ...
        if capacitance_matrix_exists == False:
            capacitance_matrix = np.zeros((3,3)).astype(complex)
        
        capacitance_matrix_exists = True
        
        i, j = line_direct_lookup[property_number]

        capacitance_matrix[i][j] = complex(line_config[prop_name]) * 376.9911 * 1e-9 * 1j #Convert from nF/mile to Admittance

    return capacitance_matrix, capacitance_matrix_exists
